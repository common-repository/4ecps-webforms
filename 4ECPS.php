...
/**
 * Plugin Name: 4ECPS Webforms
 * Description: This adds the 4ECPS Marketing webforms to your website.
 * Version: 0.2.18
 * Author: jumpdemand
...
define(__NAMESPACE__ . '\ACTIVEDEMAND_VER', '0.2.18');
define(__NAMESPACE__."\PLUGIN_VENDOR", "4ECPS");
define(__NAMESPACE__."\PLUGIN_VENDOR_LINK", "https://4ecps.com/");
define(__NAMESPACE__."\PREFIX", 'ecps');
...
function activedemand_send_stale_carts($stale_carts)
{
    ...
    $url = activedemand_api_url("v1/forms/$activedemand_form_id");
...
function api_delete_post($request)
{
    ...
    if (!isset($parameters['api_key']) || strcmp($parameters['api_key'], activedemand_api_key()) != 0) {
...
function active_demand_recover_cart()
{
    ...
    if ( isset($_GET['recover-cart']) && $cookie_cart_id = sanitize_text_field($_GET['recover-cart']) ) {
        $id_cart = $wpdb->get_var('SELECT id_cart FROM '.$wpdb->prefix.'cart WHERE cookie_cart_id = "'.$cookie_cart_id.'"');        
        if ($id_cart) {
            $products_to_recover = $wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'cart_product WHERE id_cart = '.(int)$id_cart);                        
    ...
    foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
        ...
        if (strpos($current_url, 'cart-key') == false) {
            ...
            if (!$cart_product_id) {
                ...
                $wpdb->query($wpdb->prepare("UPDATE " . $wpdb->prefix . "cart_product SET quantity = " . $quantity . " WHERE  id_product = " . $id_product . ' AND id_product_variation = ' . (int)$variation_id . ' AND id_cart = ' . $id_cart));
            }
        }
    }
}
    ...
    if (!is_admin() && !current_user_can('administrator')) {
        if (get_option(PREFIX . '_enable_access_control') == 1) {
            ...
            $get_results_match = $wpdb->get_results(
                ...
                "(
                    url = '$current_url_param' OR '$current_url_param' LIKE REPLACE(url, '.*', '%')
                )")
            );
...
function get_access_login_status($object_id)
{
    ...
    if (isset($_COOKIE['acf_session_' . $object_id]) && !isset($_COOKIE['acf_access_login_status_' . $object_id])) {
        ...
        if ((isset($login_response->{'login-at'}) && !empty($login_response->{'login-at'}))){
           ...
           setcookie('acf_access_login_status_' . $object_id, $loggin_status, 0, "/", $basedomain);
        }
